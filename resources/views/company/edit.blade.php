<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('company.update',['company'=>$company->id,'redirectToList'=>request('redirectToList')]) }}" class="space-y-8">
                @csrf
                @method("PUT")
                {{-- Company Card --}}
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="company-industry" name="company[industry]"
                                    class="mt-2 w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                                    required>
                                <option value="" disabled {{ old('company.industry',$company->industry) ? '' : 'selected' }}>Select…</option>
                                @foreach($industries as $ind)
                                    <option value="{{ $ind }}" {{ old('company.industry',$company->industry)===$ind ? 'selected':'' }}>
                                        {{ $ind }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input id="owner-name" type="text" name="owner[name]"
                                   value="{{ old('owner.name',$company->owner->name) }}"
                                   class="mt-2 w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                                   required>
                            @error('owner.name')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input id="owner-email" type="email" name="owner[email]"
                                   value="{{ old('owner.email',$company->owner->email) }}"
                                   class="mt-2 w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm bg-gray-200"
                                   required disabled>
                            @error('owner.email')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
