    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                <a href="{{ route('job-vacancy.edit', ['job_vacancy'=>$job_vacancy->id,"redirectToList"=>"false"]) }}"
                ...
                <form method="POST" action="{{ route('job-vacancy.destroy', $job_vacancy->id) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-2xl border border-gray-100 p-6">
                ...
                <!-- Job Vacancy info -->
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-gray-800">Job Vacancy Details</h3>
                    <p class="text-sm text-gray-600 mt-2">
                        <span class="font-medium">Company:</span> {{ $job_vacancy->company->name ?? '—' }}<br>
                        <span class="font-medium">location:</span> {{ $job_vacancy->location ?? '—' }}<br>
                        <span class="font-medium">type:</span> {{ $job_vacancy->type ?? '—' }}<br> 
                        <span class="font-medium">salary:</span>$ {{ number_format( $job_vacancy->salary,2) ?? '—' }}<br>                        
                        <span class="font-medium">type:</span> {{ $job_vacancy->description ?? '—' }}<br> 
                    </p>
                </div>

                <!-- Tabs Navigation -->
                @php
                    $tab = request('tab', 'applications'); // default to jobs
                @endphp
                ...
                @if ($tab === 'applications')
                    ...
                    <div>
                        ...
                        @if ($job_vacancy->jobApplications->count())
                            <table class="min-w-full bg-white border border-gray-100 rounded-lg shadow-sm">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach ($job_vacancy->jobApplications as $app)
                        ...
                        @else
                            <p class="text-gray-500 text-sm">No applications found for this vacancy.</p>
                        @endif
                    </div>
                @else
